...
start:
	for frame.ip != len(stream) {
		...
		case op.OpAdd:
			...
			if err != nil {
				vm.runtimeError("Cannot add %s and %s: %v", left.Type(), right.Type(), err)
			}
			vm.Push(res)
		...
		case op.OpSub:
			...
			if err != nil {
				vm.runtimeError("Cannot subtract %s from %s: %v", right.Type(), left.Type(), err)
			}
			vm.Push(res)
		...
		case op.OpMul:
			...
			if err != nil {
				vm.runtimeError("Cannot multiply %s by %s: %v", left.Type(), right.Type(), err)
			}
			vm.Push(res)

		case op.OpMod:
			right, left := vm.Pop(), vm.Pop()
			res, err := object.Mod(left, right)
			if err != nil {
				vm.runtimeError("Cannot perform modulo on %s and %s: %v", left.Type(), right.Type(), err)
			}
			vm.Push(res)
		...
		case op.OpDiv:
			...
			if err != nil {
				vm.runtimeError("Cannot divide %s by %s: %v", left.Type(), right.Type(), err)
			}
			vm.Push(res)
		...
		case op.OpMap:
			length := int(stream[frame.ip+1])
			frame.ip++
			m := object.Map{Map: map[string]object.Object{}}
			for i := 0; i < length; i++ {
	...
	frame = nil
	stream = nil
	vm.openUpValues = []*object.UpValue{}
