...
func (c *Compiler) Declare(name string) {
	...
	if c.target.scopeDepth == 0 && c.target.targetType == SCRIPT_TARGET {
		...
		if index != -1 {
			...
			c.CompilationError(0, "Error: %v is already declared", name)
		}
		c.globals = append(c.globals, name)
...
func (c *Compiler) GetIdentifier(name token.Token) {
	...
	c.CompilationError(name.Line, "Error: Undeclared Identifier: %v", name.Literal)
}
...
func (c *Compiler) CompilationError(line int, format string, args ...any) {
	errMessage := fmt.Sprintf(format+"\n", args...)

	// Show stack trace of compilation targets
	target := c.target
	for target.outer != nil {
		target = target.outer
	}
	errMessage = fmt.Sprintf("Line %d of %s:%d\n",
		line,
		target.function.ScriptName,
		line,
	)

	fmt.Println(colors.Colorize("Compilation Error:", colors.RED))
